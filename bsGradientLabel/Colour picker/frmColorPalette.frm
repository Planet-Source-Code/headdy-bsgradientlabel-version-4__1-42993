    ...
    Select Case ButId
        ...
        Case 2 To 49
            If Not IsSystemColors Then
                Clrs(ButId).Clr = CLng(VBA.Mid(NorClrVal, (ButId - 2) * 8 + 1, 8))
                Clrs(ButId).Tip = ""
            Else
                If (ButId <= 26) Then
                    Clrs(ButId).Clr = CLng(VBA.Mid(SysClrVal, (ButId - 2) * 10 + 1, 10))
                    Clrs(ButId).Tip = Trim(VBA.Mid(SysClrTip, (ButId - 2) * 23 + 1, 23))
                Else
...
Private Function ShowColor() As Long
    ...
    With ClrInf
        ...
        .lpCustColors = VBA.StrConv(CustomColors, vbUnicode)       'Array of 16 byte values
